...
class Socket extends BaseClass {
     ...
     * @param msg 消息发送接受处理类
     */
    public initServer(host: string, port: number, userID: number): void {
        this._host = host;
        this._port = port;
        this.userID = userID
    }
    ...
    public connect(): void {
        ...
        this._socket.connectByUrl(`ws://${this._host}:${this._port}/ws/zmg?zmgUserId=${this.userID}`) //ws://192.168.0.15:9914/ws/zmg?zmgUserId=2
        this.addEvents();
