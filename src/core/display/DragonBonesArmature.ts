class DragonBonesArmature extends egret.DisplayObjectContainer {
        ...
        if (displayObject) {
            if (this._cacheAllSlotDisplayData) {
                ...
                if (cacheDisplayData) {
                    ...
                    // displayObject.width = cacheDisplayData.width
                    // displayObject.height = cacheDisplayData.height
                }
            } else {
                let oldDisplayObject = slot.display
                if (oldDisplayObject) {
                    ...
                    // displayObject.width = oldDisplayObject.width
                    // displayObject.height = oldDisplayObject.height
                }
            }
        }
        slot.display = displayObject;
    }
    ...
    public getAllSlotDisplayData(): any {
        ...
        for (let i = 0, len = slots.length; i < len; i++) {
            let slot = slots[i];
            let displayObject = slot.display()
            result[slot.name] = {
                x: displayObject.x,
                y: displayObject.y,
